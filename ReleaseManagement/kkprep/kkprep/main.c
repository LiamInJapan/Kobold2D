...
int main (int argc, const char * argv[])
{
	...
	// run package maker
	{
		...
		NSString* resDir = [NSString stringWithFormat:@"%@/Resources", pkgContentsDir, startDir];
		...
		NSString* pkgFile = @"Kobold2D.pkg";
		if (isTestMode) {
			pkgFile = @"Kobold2D-TESTMODE.pkg";
		}
		...
		NSString* pkgFileRoot = [NSString stringWithFormat:@"%@/%@", packageDir, pkgFile];
		if ([fileManager moveItemAtPath:pkgFile toPath:pkgFileRoot error:&error] == NO)
			showAlertWithError(error);
